...
echo "กำลังนำเข้าข้อมูล (sac_data)...\n";
...
foreach ($all_rows as $result_sqlsvr) {
    ...
    $values_db[] = "('$product_key','$product_id','$pgroup_id','$name_t','$brand_id','$price_code','$price','$unit_name','$group_desc')";

    if ($current % $batch_size == 0 || $current == $total_rows) {
        $values_str_db = implode(",", $values_db);

        try {
            $sql = "INSERT INTO ims_product (product_key,product_id,pgroup_id,name_t,brand_id,price_code,price,unit_name,group_desc) 
                    VALUES $values_str_db
            ...
            $error_count += count($values_db);
            error_log("Error importing to sac_data: " . $e->getMessage());
            echo "Error sac_data: " . $e->getMessage() . "\n";
        }

        $insert_update_rows = count($values_db);
...
echo "เกิดข้อผิดพลาด sac_data: $error_count รายการ\n";

echo "sac_data count: " . $conn->query("SELECT COUNT(*) FROM ims_product")->fetchColumn() . "\n";
